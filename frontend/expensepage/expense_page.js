...
function getformvalue(event) {
    ...
    axios.post('http://16.171.46.78:4000/user/add-expense', expense_detail, { headers: { 'Authorization': token } })
        .then((response) => {
...
window.addEventListener('DOMContentLoaded', () => {
    if (decodedtoken.ispremium == true) {
        ...
        document.getElementById('dbtn').onclick=async()=>{
          try{
              const response=await  axios.get('http://16.171.46.78:4000/premium/download',{ headers: { 'Authorization': token } })
    ...
    const page=1;
    axios.get(`http://16.171.46.78:4000/user/get-expense/${no_of_item}?page=${page}`, { headers: { 'Authorization': token } })
        .then((response) => {
    ...
    function showleaderboard() {
     ...
     inputElement.onclick=async()=>{
        const token=localStorage.getItem('token');
    const leaderboardarray= await axios.get('http://16.171.46.78:4000/premium/show-leaderboard',{ headers: {'Authorization':token}})
...
function showscreen(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        ul.appendChild(li);
        dltbtn.onclick = () => {
            axios.delete(`http://16.171.46.78:4000/user/delete-expense/${data[i].id}`,{ headers: { 'Authorization': token }},{amount:data.amount})
                .then((result) => {
                    console.log("deleted");
                    ul.removeChild(li);
...
function showscreenadd(data) {
    ...
    ul.appendChild(li);
    dltbtn.onclick = () => {
        axios.delete(`http://16.171.46.78:4000/user/delete-expense/${data.id}`,{ headers: { 'Authorization': token }},{amount:data.amount})
            .then((result) => {
                console.log("deleted");
                ul.removeChild(li);
            }).catch(err => console.log(err));
...
pbtn.onclick = async (e) => {
    e.preventDefault();
    const response = await axios.get('http://16.171.46.78:4000/premium/purchase-premium', { headers: { "Authorization": token } });
    let options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id,
        "handler": async function (response) {
            const res = await axios.post('http://16.171.46.78:4000/premium/update-transaction', {
    ...
    rpz1.on('payment.failed', async function (response) {
        await axios.post('http://16.171.46.78:4000/premium/update-transaction', {
            order_id: options.order_id,
            payment_id: response.razorpay_payment_id
        }, { headers: { "Authorization": token } })
